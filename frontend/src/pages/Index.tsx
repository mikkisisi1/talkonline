...
const Index = () => {
  ...
  const playTapChime = useCallback(async () => {
    try {
      ...
      // Нежный хрустальный звоночек - три гармоники
      // Основной тон - мягкий
      const osc1 = ctx.createOscillator();
      const gain1 = ctx.createGain();
      osc1.type = 'sine';
      osc1.frequency.setValueAtTime(523, now); // C5 - нота До
      osc1.frequency.exponentialRampToValueAtTime(659, now + 0.15); // плавно к E5
      ...
      // Вторая гармоника - октава выше, тише
      const osc2 = ctx.createOscillator();
      const gain2 = ctx.createGain();
      osc2.type = 'sine';
      osc2.frequency.setValueAtTime(1047, now); // C6
      ...
      // Третья гармоника - ещё выше, еле слышно - добавляет "блеск"
      const osc3 = ctx.createOscillator();
      const gain3 = ctx.createGain();
      osc3.type = 'sine';
      osc3.frequency.setValueAtTime(2093, now); // C7
